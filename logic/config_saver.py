...
def save_barbers(barbers: dict):
    ...
    if file.exists():
        print("config_saver.py>Guardando Barberos")
        with open(file, "w", encoding="utf-8") as f:
            json.dump(barbers, f, indent=4, ensure_ascii=False)
    else:
        print("config_saver.py>Guardando Barberos en un archivo nuevo.")
...
def save_services(services: dict):
    ...
    if file.exists():
        print("config_saver.py>Guardando Servicios")
        with open(file, "w", encoding="utf-8") as f:
            json.dump(services, f, indent=4, ensure_ascii=False)
    else:
        print("config_saver.py>Guardando Servicios en un archivo nuevo")
...
def save_payment_methods(payment_methods: dict):
    ...
    if file.exists():
        print("config_saver.py>Guardando Métodos de Pago")
        with open(file, "w", encoding="utf-8") as f:
            json.dump(payment_methods, f, indent=4, ensure_ascii=False)
    else:
        print("config_saver.py>Guardando Métodos de Pago en un archivo nuevo.")
        file.parent.mkdir(parents=True, exist_ok=True)
        with open(file, "w", encoding="utf-8") as f:
            json.dump(payment_methods, f, indent=4, ensure_ascii=False)